  ...
  Widget build(BuildContext context) {
    ...
    final cardPreview = Container(
      ...
      child: Column(
        ...
        children: [
          const Text('\$50 off', style: TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
          const SizedBox(height: 4),
          const Text('on your first order', style: TextStyle(color: Colors.white70)),
          const Spacer(),
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text('Promo code valid for orders over \$150.', style: TextStyle(color: Colors.black54)),
            const SizedBox(height: 16),
            Row(
              children: [
                const Expanded(child: Text('Payment information', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600))),
            ...
            Builder(builder: (context) {
              final digits = widget.data.cardNumber.replaceAll(RegExp(r'[^0-9]'), '');
              final brand = _brandFromNumber(digits);
              final ending = digits.isNotEmpty ? digits.substring(digits.length - 2) : '';
              Widget badge;
              if (digits.startsWith('5')) {
                ...
                badge = const Text('VISA', style: TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF1A1F71)));
              } else if (digits.startsWith('3')) {
                badge = const Text('AMEX', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.teal));
              } else {
                badge = const Icon(Icons.credit_card, size: 18, color: Colors.orange);
              }
              return Row(
                children: [
                  badge,
                  const SizedBox(width: 8),
                  Expanded(child: Text('Card holder\n$brand ending **$ending', style: const TextStyle(fontSize: 14))),
                ],
              );
            }),
            const SizedBox(height: 16),
            ...
            TextField(
              ...
              decoration: const InputDecoration(hintText: 'PROMO20-08', border: OutlineInputBorder()),
            ),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: _processing ? const CircularProgressIndicator(color: Colors.white) : const Text('Pay'),
              ),
            ),
          ],
        ),
